...
const translations = {
    ...
    "Scaling Master | Agentic Architect | SK하이닉스 유지보수 엔지니어": "Scaling Master | Agentic Architect | Hynix Maintenance Engineer",
    ...
    "<strong>Python 데이터 분석:</strong> Scikit-learn, TensorFlow 기반 AI 알고리즘 구현 및 로봇 제어 응용.": "<strong>Python Data Analysis:</strong> Implementation of AI algorithms based on Scikit-learn, TensorFlow, and robot control applications.",
    "<strong>C/C++ 임베디드 최적화:</strong> 객체 지향 개념을 활용한 하드웨어 제어 및 효율적 메모리 관리 코딩.": "<strong>C/C++ Embedded Optimization:</strong> Hardware control and efficient memory management coding utilizing object-oriented concepts.",
    "장치 제어 (Arduino)": "Device Control (Arduino)",
    "<strong>지능형 액추에이터 제어:</strong> 센서 데이터 기반 Servo, DC, 스테퍼 모터 복합 구동 시스템 설계.": "<strong>Intelligent Actuator Control:</strong> Sensor data-based Servo, DC, stepper motor complex drive system design.",
    "<strong>상태 머신(State Machine):</strong> Non-blocking 기법을 활용한 오류 없는 고밀도 제어 로직 구현.": "<strong>State Machine:</strong> Implementation of error-free high-density control logic utilizing non-blocking techniques.",
    "공정 자동화 (PLC)": "Automation (PLC)",
    "<strong>시퀀스 제어:</strong> GX Works3 기반 공정 자동화 설비 운영 및 이상 유무 진단/유지보수.": "<strong>Sequence Control:</strong> Operation of process automation facilities based on GX Works3 and diagnosis/maintenance of abnormalities.",
    "<strong>실무 자동화:</strong> 공기압 실린더 및 솔레노이드 밸브 연동 제어 등 산업 현장 설비 운영 역량.": "<strong>Practical Automation:</strong> Ability to operate industrial field facilities such as pneumatic cylinder and solenoid valve interlocking control.",
    "반도체 기초 지식": "Semiconductor Basics",
    "<strong>회로 설계 및 분석:</strong> TR, FET의 동작 원리를 활용한 스위칭 회로 설계 및 계측 검증.": "<strong>Circuit Design & Analysis:</strong> Switching circuit design and measurement verification utilizing the operating principles of TR and FET.",
    "<strong>디지털 논리 설계:</strong> 순차 논리 회로 시뮬레이션 및 정밀 하드웨어 로직 구성 능력 보유.": "<strong>Digital Logic Design:</strong> Sequential logic circuit simulation and precision hardware logic configuration capability.",
    ...
    "원인 분석 논리": "The Diagnostic Logic",
    "<strong>[이슈 1] 모터 토크 부족</strong>": "<strong>[ISSUE 1] Motor Torque Shortage</strong>",
    ...
    "<strong>[이슈 2] 접지력 및 마찰력 결함</strong>": "<strong>[ISSUE 2] Traction and Friction Defects</strong>",
    ...
    "<strong>[이슈 3] 링크 간 간섭 및 마모</strong>": "<strong>[ISSUE 3] Interference and Wear between Links</strong>",
    "소음 및 부품 마모 원인 추적<br>→ AutoCAD 도면 재검토 및 각 조인트의 수평 정밀 정렬을 통한 마찰력 제로화": "Tracked causes of noise and part wear<br>→ AutoCAD drawing review and horizontal precision alignment of each joint",
    "체계적 검증": "Systematic Verification",
    ...
    "AutoCAD-실물 동기화 보고서": "AutoCAD-PHYSICAL SYNC REPORT",
    ...
    "이메일": "Email",
    "내용": "Content",
    "실시간 설비 진단 및 유지보수 시뮬레이션": "Real-time Equipment Diagnosis & Maintenance Simulation",
    "저장하기": "Save",
    "취소": "Cancel",
...
function applyTranslation(lang) {
    ...
    // Sort elements by DOM depth descending to replace inner elements before outer elements 
    // Wait, innerHTML replacement is just exact string match. So depth doesn't matter too much
    // but just checking every element exact match will suffice.
    elements.forEach(el => {
        const cleanHTML = el.innerHTML.trim();
        if (dict[cleanHTML]) {
            el.innerHTML = dict[cleanHTML];
        }
    });
